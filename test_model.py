...
# Training Data
test_data = [
    # Valid URLs - finance, ACFR, budget, financial Reports etc.
    (
...
df_test = pd.DataFrame(test_data, columns=["url", "anchor_text", "label"])
...
if __name__ == "__main__":
    ...
    # Get predictions
    ranked_df = ranker.rank_urls(
        df_test["url"].tolist(), df_test["anchor_text"].tolist()
    )

    # Convert scores to binary preds. (Threshold = 0.9)
    ranked_df["predicted_label"] = (ranked_df["score"] >= 0.9).astype(int)

    # Evaluate performance
    accuracy = accuracy_score(df_test["label"], ranked_df["predicted_label"])
    report = classification_report(
        df_test["label"],
        ranked_df["predicted_label"],
    ...
    print(ranked_df[["url", "anchor_text", "score", "predicted_label"]].head(10))
